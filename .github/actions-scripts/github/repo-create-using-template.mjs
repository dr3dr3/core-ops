...
console.assert(process.env.REPO_TEMPLATE_OWNER, "REPO_TEMPLATE_OWNER not present");
console.assert(process.env.REPO_TEMPLATE_NAME, "REPO_TEMPLATE_NAME not present");
...
async function createRepoUsingTemplate() {
    ...
    try {
        const { status:repoCreated } = await octokit.rest.repos.createUsingTemplate({
            template_owner: process.env.REPO_TEMPLATE_OWNER,
            template_repo: process.env.REPO_TEMPLATE_NAME,
